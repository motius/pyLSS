...
def genericWrite(id, cmd, param=None):
    ...
    if param is None:
        LSS.bus.write(
            (lssc.LSS_CommandStart + str(id) + cmd + lssc.LSS_CommandEnd).encode()
        )
    else:
        LSS.bus.write(
            (
                lssc.LSS_CommandStart + str(id) + cmd + str(param) + lssc.LSS_CommandEnd
            ).encode()
        )
    return True
    ...
    try:
        ...
        while c.decode("utf-8") != lssc.LSS_CommandReplyStart:
            ...
            if c.decode("utf-8") == "":
                break
        # Get packet
        data = LSS.bus.read_until(
            lssc.LSS_CommandEnd.encode("utf-8")
        )  # Otherwise (without ".encode('utf-8')") the received LSS_CommandEnd is not recognized by read_until, making it wait until timeout.
        # Parse packet
        matches = re.match(
            "(\d{1,3})([A-Z]{1,4})(-?\d{1,18})", data.decode("utf-8"), re.I
        )
        # Check if matches are found
        if matches is None:
            return None
        if (
            (matches.group(1) is None)
            or (matches.group(2) is None)
            or (matches.group(3) is None)
        ):
            return None
...
class LSS:
    ...
    def getIsFirstPositionEnabled(self):
        genericWrite(self.servoID, lssc.LSS_QueryFirstPosition)
        return (
            genericRead_Blocking_int(self.servoID, lssc.LSS_QueryFirstPosition)
            is not None
        )
    ...
    def getAngularHoldingStiffness(self, queryType=lssc.LSS_QuerySession):
        ...
        return genericRead_Blocking_int(
            self.servoID, lssc.LSS_QueryAngularHoldingStiffness
        )
    ...
    def setAngularHoldingStiffness(self, value, setType=lssc.LSS_SetSession):
        if setType == lssc.LSS_SetSession:
            return genericWrite(
                self.servoID, lssc.LSS_ActionAngularHoldingStiffness, value
            )
        elif setType == lssc.LSS_SetConfig:
            return genericWrite(
                self.servoID, lssc.LSS_ConfigAngularHoldingStiffness, value
            )
